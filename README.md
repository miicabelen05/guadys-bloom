# Guady´s Bloom

## Descripción
Guady’s Bloom es un sitio web desarrollado como proyecto integrador del curso de Desarrollo Web.
Se trata de una marca ficticia de cosmética natural artesanal, enfocada en una estética delicada, moderna y responsive.

El proyecto incluye múltiples páginas HTML estructuradas con una arquitectura organizada en SASS, aplicando buenas prácticas de maquetado y diseño responsive.

## Tecnologías utilizadas
- HTML5
- CSS3
- SASS (partials, variables, mixins, nesting)
- Flexbox
- Grid
- Bootstrap (solo para componentes específicos como navbar)
- Bootstrap Icons
- Formspree (integración de formulario funcional)

## Características principales
    ✔ Diseño responsive completo (mobile-first adaptado)
	✔ Arquitectura SASS modular
	✔ Uso de variables, mixins y nesting
	✔ Animaciones con @keyframes
	✔ Maquetación con Grid y Flexbox
	✔ Formulario de contacto funcional
	✔ Integración de GitHub Pages
	✔ Estética coherente con identidad de marca

## Estructura del proyecto
- `index.html`
- `README.md`

- `/pages`
    - `productos.html`
    - `contacto.html`
- `/css`
    - `estilos.css`
    - `estilos.css.map`
- `/scss`
    - _base.scss
    - _components.scss
    - _layout.scss
    - _mixins.scss
    - _responsive.scss
    - _variables.scss
    - _main.scss
- `/img`
    - fondo.jpg
    - imagen1.jpg
    - imagen2.jpg
    - imagen3.jpg
    - logo.jpg
- `/video`
    - El video de representación esta externo (Google Drive)

## Responsive
El sitio se adapta completamente a dispositivos móviles mediante media queries y uso de Grid/Flexbox, evitando scroll horizontal y asegurando correcta visualización en diferentes resoluciones.

## Sitio online
GitHub Pages:  
    https://miicabelen05.github.io/guadys-bloom/

## Autor/a
- Gallego Micaela Belen